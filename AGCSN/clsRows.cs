...
namespace AGCSN
{
    public class clsRows : IEnumerable
	{
        ...
        internal clsRows(ActiveGanttCSNCtl oControl)
		{
		...
		internal void NodesDraw()
		{
			...
			for (lIndex = mp_lRealFirstVisibleRow; lIndex <= Count; lIndex++) 
			{
                ...
                if (oRow.ClientAreaVisibility == E_CLIENTAREAVISIBILITY.VS_INSIDEVISIBLEAREA & oNode.Hidden == false) 
				{
                    ...
                    {
                        oNode.mp_lTextLeft = (int) mp_oControl.clsG.mp_oTextFinalLayout.Left;
                        oNode.mp_lTextTop = (int) mp_oControl.clsG.mp_oTextFinalLayout.Top;
                        oNode.mp_lTextRight = (int)mp_oControl.clsG.mp_oTextFinalLayout.Left + (int)mp_oControl.clsG.mp_oTextFinalLayout.Width - 1;
                        oNode.mp_lTextBottom = (int)mp_oControl.clsG.mp_oTextFinalLayout.Top + (int)mp_oControl.clsG.mp_oTextFinalLayout.Height - 1;
                    }
                    else
                    {

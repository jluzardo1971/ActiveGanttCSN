...
namespace AGCSN
{
    public class clsColumns : IEnumerable
	{
        ...
        public clsColumns(ActiveGanttCSNCtl oControl)
		{
			...
			for (lIndex = 1;lIndex <= Count;lIndex++) 
			{
				...
				if (oColumn.Visible == true)
				{
                    ...
                    if (mp_oControl.DrawEventArgs.CustomDraw == false)
					{
                        ...
                        {
                            oColumn.mp_lTextLeft = (int)mp_oControl.clsG.mp_oTextFinalLayout.Left;
                            oColumn.mp_lTextTop = (int)mp_oControl.clsG.mp_oTextFinalLayout.Top;
                            oColumn.mp_lTextRight = (int)mp_oControl.clsG.mp_oTextFinalLayout.Left + (int)mp_oControl.clsG.mp_oTextFinalLayout.Width - 1;
                            oColumn.mp_lTextBottom = (int)mp_oControl.clsG.mp_oTextFinalLayout.Top + (int)mp_oControl.clsG.mp_oTextFinalLayout.Height - 1;
                        }
                        else
                        {

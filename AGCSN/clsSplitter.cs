...
namespace AGCSN
{
	public class clsSplitter
	{
        ...
        public clsSplitter(ActiveGanttCSNCtl oControl)
		{
        ...
        public void SetColor(int Index, Color oColor)
        {
            ...
            {
                mp_oControl.mp_ErrorReport(SYS_ERRORS.SPLITTER_INVALIDOP, "Invalid Operation", "ActiveGanttCSNCtl.clsSplitter.SetColor");
                return;
            }
            if (Index < 0)
            {
                mp_oControl.mp_ErrorReport(SYS_ERRORS.SPLITTER_INVALID_INDEX, "Invalid Index", "ActiveGanttCSNCtl.clsSplitter.SetColor");
                return;
            }
        ...
        public Nullable<Color> GetColor(int Index)
        {
            ...
            if (mp_yType != E_SPLITTERTYPE.SA_USERDEFINED)
            {
                mp_oControl.mp_ErrorReport(SYS_ERRORS.SPLITTER_INVALIDOP, "Invalid Operation", "ActiveGanttCSNCtl.clsSplitter.GetColor");
                return null;
            }
            if (Index < 0)
            {
                mp_oControl.mp_ErrorReport(SYS_ERRORS.SPLITTER_INVALID_INDEX, "Invalid Index", "ActiveGanttCSNCtl.clsSplitter.GetColor");
                return null;
            }
            if (Index > mp_aColors.Count - 1)
            {
        ...
        public int Width
        {
            ...
            set
            {
                ...
                else
                {
                    if (value < 0)
                    {
                        mp_oControl.mp_ErrorReport(SYS_ERRORS.SPLITTER_INVALID_WIDTH, "Invalid Width Value", "ActiveGanttCSNCtl.clsSplitter.Width");
                        return;
                    }
                    mp_lWidth = value;
                }
        ...
        internal void Draw()
        {
            ...
            {
                mp_oControl.clsG.mp_DrawItem(this.Left + 1, 0, this.Left + this.Width + 1, mp_oControl.clsG.Height(), "", "", false, null, 0, 0, this.Style);
            }
            else
            {
                int i = 0;

...
namespace AGCSN
{
	internal class clsCollectionBase
	{
		...
		public clsCollectionBase(ActiveGanttCSNCtl oControl, String sObjectName)
		{
		...
		public Object m_oItem(String v_lIndex, SYS_ERRORS v_lErr1, SYS_ERRORS v_lErr2, SYS_ERRORS v_lErr3, SYS_ERRORS v_lErr4)
		{
            ...
            if (!Globals.g_IsNumeric(v_lIndex))
			{
				if ( v_lIndex == "" )
				{
                    mp_oControl.mp_ErrorReport(v_lErr1, "Invalid " + m_sObjectName + " object key, key cannot be an empty string", "ActiveGanttCSNCtl" + m_sObjectName + "s.Get m_oItem");
					return null;
				}
				lIndex = m_lFindIndexByKey(v_lIndex);
				if ( lIndex == -1 )
				{
                    mp_oControl.mp_ErrorReport(v_lErr2, m_sObjectName + " object not found, invalid key (\"" + v_lIndex + "\")", "ActiveGanttCSNCtl" + m_sObjectName + "s.Get m_oItem");
					return null;
				}
			}
			else
			{
				...
				{
                    mp_oControl.mp_ErrorReport(v_lErr3, m_sObjectName + " object not found, invalid v_lIndex", "ActiveGanttCSNCtl" + m_sObjectName + "s.Get m_oItem");
					return null;
				}
				if ( lIndex < 1 | lIndex > mp_aoCollection.Count )
				{
                    mp_oControl.mp_ErrorReport(v_lErr4, m_sObjectName + " object not found, v_lIndex out of bounds", "ActiveGanttCSNCtl" + m_sObjectName + "s.Get m_oItem");
					return null;
				}
			}
		...
		public void m_Add(object r_oObject, String v_sKey, SYS_ERRORS v_lErr1, SYS_ERRORS v_lErr2, bool v_bKeyRequired, SYS_ERRORS v_lKeyError)
		{
			...
			{
				mp_oControl.mp_ErrorReport(v_lErr1, "Invalid " + m_sObjectName + " object key, key cannot be numeric", "ActiveGanttCSNCtl.cls" + m_sObjectName + "s.Add");
				return;
			}
			if (v_sKey != "" )
			{
				...
				{
					mp_oControl.mp_ErrorReport(v_lErr2, "Key is not unique in " + m_sObjectName + "s collection", "ActiveGanttCSNCtl.cls" + m_sObjectName + "s.Add");
					return;
				}
			}
		...
		public void m_Remove(String v_sIndex, SYS_ERRORS v_lErr1, SYS_ERRORS v_lErr2, SYS_ERRORS v_lErr3, SYS_ERRORS v_lErr4)
		{
            ...
            if (!Globals.g_IsNumeric(v_sIndex))
			{
				if ( v_sIndex == "" )
				{
                    mp_oControl.mp_ErrorReport(v_lErr1, "Invalid " + m_sObjectName + " object key, key cannot be an empty string", "ActiveGanttCSNCtl" + m_sObjectName + "s.Remove");
					return;
				}
				lIndex = m_lFindIndexByKey(v_sIndex);
				if ( lIndex == -1 )
				{
                    mp_oControl.mp_ErrorReport(v_lErr2, m_sObjectName + " object not found, invalid key (\"" + v_sIndex + "\")", "ActiveGanttCSNCtl" + m_sObjectName + "s.Remove");
					return;
				}
			...
			else
			{
				...
				{
                    mp_oControl.mp_ErrorReport(v_lErr3, m_sObjectName + " object not found, invalid v_sIndex", "ActiveGanttCSNCtl" + m_sObjectName + "s.Remove");
					return;
				}
				if ( lIndex < 1 | lIndex > mp_aoCollection.Count )
				{
                    mp_oControl.mp_ErrorReport(v_lErr4, m_sObjectName + " object not found, v_sIndex out of bounds", "ActiveGanttCSNCtl" + m_sObjectName + "s.Remove");
					return;
				}
				clsItemBase oItemBase;
		...
		public Object m_oReturnArrayElement(int r_lIndex)
		{
            ...
            {
                mp_oControl.mp_ErrorReport(SYS_ERRORS.ERR_RETARRELEMINDEX_G, "Object not found, Index out of bounds. Index: " + r_lIndex.ToString(), "ActiveGanttCSNCtl.cls" + m_sObjectName + "s.m_oReturnArrayElement");
                return null;
            }
		}
		...
		public Object m_oReturnArrayElementKey(String v_sKey)
		{
			...
			else
			{
				...
				{
                    mp_oControl.mp_ErrorReport(SYS_ERRORS.ERR_RETARRELEMKEY_G, "Key not found", "ActiveGanttCSNCtl" + m_sObjectName + "s.m_oReturnArrayElementKey");
					return null;
				}
			}
		}
		...
		public void mp_SetKey(ref String sCurrentKey, String sNewKey, SYS_ERRORS ErrNumber)
		{
			if (m_bIgnoreKeyChecks == false)
			{
				...
				{
                    mp_oControl.mp_ErrorReport(ErrNumber, "Numeric or duplicate " + m_sObjectName + " object key", "ActiveGanttCSNCtl" + m_sObjectName + ".Let Key");
					return;
				}
			}
			sCurrentKey = sNewKey;

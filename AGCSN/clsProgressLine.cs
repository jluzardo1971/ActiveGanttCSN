...
namespace AGCSN
{
    public class clsProgressLine
    {
        ...
        internal clsProgressLine(ActiveGanttCSNCtl oControl)
        {
        ...
        public void SetColor(int Index, Color oColor)
        {
            ...
            {
                mp_oControl.mp_ErrorReport(SYS_ERRORS.PROGRESSLINE_INVALIDOP, "Invalid Operation", "ActiveGanttCSNCtl.clsProgressLine.SetColor");
                return;
            }
            if (Index < 0)
            {
                mp_oControl.mp_ErrorReport(SYS_ERRORS.PROGRESSLINE_INVALID_INDEX, "Invalid Index", "ActiveGanttCSNCtl.clsProgressLine.SetColor");
                return;
            }
        ...
        public Nullable<Color> GetColor(int Index)
        {
            ...
            if (mp_yLineStyle != E_PROGRESSLINESTYLE.PLT_USERDEFINED)
            {
                mp_oControl.mp_ErrorReport(SYS_ERRORS.PROGRESSLINE_INVALIDOP, "Invalid Operation", "ActiveGanttCSNCtl.clsProgressLine.GetColor");
                return null;
            }
            if (Index < 0)
            {
                mp_oControl.mp_ErrorReport(SYS_ERRORS.PROGRESSLINE_INVALID_INDEX, "Invalid Index", "ActiveGanttCSNCtl.clsProgressLine.GetColor");
                return null;
            }
            if (Index > mp_aColors.Count - 1)
            {
            ...
            return (System.Drawing.Color)mp_aColors[Index];
        }
        ...
        public int Width
        {
            ...
            set
            {
                ...
                else
                {
                    if (value < 0)
                    {
                        mp_oControl.mp_ErrorReport(SYS_ERRORS.PROGRESSLINE_INVALID_WIDTH, "Invalid Width Value", "ActiveGanttCSNCtl.clsProgressLine.Width");
                        return;
                    }
                    mp_lWidth = value;
                }
